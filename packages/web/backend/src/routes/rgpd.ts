...
import { dataExportMiddleware, dataDeletionMiddleware } from '../middlewares/rgpdMiddleware';
 ...
 * @access  Private
 */
router.get('/export', authenticate, dataExportMiddleware, (req: Request, res: Response) => {
  try {
 ...
 * @access  Private
 */
router.post('/delete', authenticate, dataDeletionMiddleware, (req: Request, res: Response) => {
  try {
    // Get the user from the authenticated request
    const user = (req as any).user;
